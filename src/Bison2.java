...
public class Bison2 extends IJPanel implements Runnable {
	Image source;
	Image source2;
	Image image;
	Image[] base;	// 기본
	...
	Bison2() {
		...
		try {
			...
			for(int i=0; i<base.length; i++) {
				...
				t.getGraphics().drawImage(source, 110, 0, 0, 100, 100*i, 0, 99+99*i ,100, this);
				base[i] = t;
			}
			move1 = new Image[2];
			for(int i=0; i<move1.length; i++) {
					Image t = new BufferedImage(140, 140, BufferedImage.TRANSLUCENT); // 542 ,140, 
					t.getGraphics().drawImage(source2, 140, 0, 0, 140, 140*i, 542, 140+140*i ,542+140, this);
				move1[i] = t;
			}
			move2 = new Image[2];
			for(int i=0; i<move2.length; i++) {
				Image t = new BufferedImage(145, 130, BufferedImage.TRANSLUCENT); 
				t.getGraphics().drawImage(source2, 145, 0, 0, 130, 294+(145*i), 552, 294+(145+145*i) ,552+132, this);
				move2[i] = t;
			}
			attack = new Image[6];
			for(int i=0; i<attack.length; i++) {
				Image t = new BufferedImage(140, 85, BufferedImage.TRANSLUCENT); //137.85
				t.getGraphics().drawImage(source, 140, 0, 0, 85, 137*i, 480,136+136*i ,480+90, this);
				attack[i] = t;
			}
			kick = new Image[5];
			for(int i=0; i<kick.length; i++) {
				Image t = new BufferedImage(130, 85, BufferedImage.TRANSLUCENT); //128,83
				t.getGraphics().drawImage(source, 130, 0, 0, 85, 128*i, 830, 128+128*i ,830+90, this);
				kick[i] = t;
			}
			uppercut = new Image[6];
			for(int i=0; i<uppercut.length; i++) {
				Image t = new BufferedImage(110, 100, BufferedImage.TRANSLUCENT); 
				t.getGraphics().drawImage(source, 110, 0, 0, 100, 5+(100*i), 563, 100+100*i ,563+100, this);
				uppercut[i] = t;
			}
			...
			for(int i=0; i<damage.length; i++) {
				...
				t.getGraphics().drawImage(source2, 166, 0, 0, 115, 166*i, 2472, 166+166*i ,2472+120, this);
				damage[i] = t;
			}
			death = new Image[4];
			for(int i=0; i<death.length; i++) {
				Image t = new BufferedImage(130, 90, BufferedImage.TRANSLUCENT);
				t.getGraphics().drawImage(source, 130, 0, 0, 90, 123*i, 1755, 123+123*i ,1755+90, this);
				death[i] = t;
			}

...
public class Ken extends IJPanel implements Runnable {
	Image source;
	Image source2;
	Image image;
	Image[] base;	// 기본
	...
	Ken() {
		...
		try { // 전체 이미지 로딩
			...
			for (int i = 0; i < base.length; i++) {
				...
				t.getGraphics().drawImage(source, 0, 0, 59, 103, 573 + (65 * i), 267, 632 + (65 * i), 370, this);
				base[i] = t;
			}
			move = new Image[6];
			for (int i = 0; i < move.length; i++) {
				Image t = new BufferedImage(59, 103, BufferedImage.TRANSLUCENT);
				t.getGraphics().drawImage(source, 0, 0, 59, 103, 30 + (50 * i), 267, 73 + (50 * i), 370, this);
				move[i] = t;
			}
			attack = new Image[4];//260,465    100,100
			for (int i = 0; i < attack.length; i++) {
				Image t = new BufferedImage(100, 100, BufferedImage.TRANSLUCENT);
				t.getGraphics().drawImage(source2, 0, 0, 100, 100, 270 + (100 * i), 460, 270 + (100+100 * i), 560, this);
				attack[i] = t;
			}
			kick = new Image[3]; //25,650  100,100
			for (int i = 0; i < kick.length; i++) {
				Image t = new BufferedImage(100, 100, BufferedImage.TRANSLUCENT);
				t.getGraphics().drawImage(source2, 0, 0, 100, 100, 25 + (100 * i), 650, 25 + (100+100 * i), 750, this);
				kick[i] = t;
			}
			uppercut = new Image[6]; //330 ,1000       75
			for (int i = 0; i < uppercut.length; i++) {
				Image t = new BufferedImage(80, 70, BufferedImage.TRANSLUCENT);
				t.getGraphics().drawImage(source2, 0, 0, 80, 70, 330 + (90 * i), 1000, 330 + (90+90 * i), 1090, this);
				uppercut[i] = t;
			}
			damage = new Image[4];
			for (int i = 0; i < damage.length; i++) {
				Image t = new BufferedImage(80, 100, BufferedImage.TRANSLUCENT);//20,2547
				t.getGraphics().drawImage(source, 0, 0, 80, 100, 20 + (77 * i), 2547, 20 + (75+75 * i), 2547+96, this);
				damage[i] = t;
			}
			death = new Image[2];//966,2566    115,80
			for (int i = 0; i < death.length; i++) {
				Image t = new BufferedImage(100, 70, BufferedImage.TRANSLUCENT);//966,2566     80
				t.getGraphics().drawImage(source2, 0, 0, 100, 70, 966 + (115 * i),2566  ,966+ (115+155 * i),2566+80 , this);
				death[i] = t;
			}
		} catch (Exception e) {
